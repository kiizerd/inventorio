...
init_inventory_view_window(struct nk_context *ctx, struct inv_inventory *inv)
{
  ...
  if (nk_begin(ctx, "Inventory View", inv_wnd_nk_rect, inv_wnd_nk_flags)) {    
    ...
    if (nk_group_begin(ctx, "Product table", table_flags))
    {
      ...
      {
        int sort_by[5];
        const float col_header_ratio[6] = {
        ...
        nk_layout_row(ctx, NK_STATIC, 16, 6, col_header_ratio);      
...
init_new_inventory_window(struct nk_context *ctx)
{
  ...
  if (nk_begin(ctx, "New Inventory", nk_rect(10, 10, 270, 400), new_inv_flags))
  {
    ...
    static const float ratio2[] = {130, 100};
    ...
    if (nk_group_begin(ctx, "Tags:", group_flags))
    {
      ...
      for (i = 0; i < tag_count; i ++) {
        ...
        if (nk_contextual_begin(ctx, 0, nk_vec2(100, 300), bounds)) {
          nk_layout_row_static(ctx, 15, 25, 1);
          // nk_button_image_label() // TODO - Add a trash can icon and delete_tag() function
    ...
    if (nk_button_label(ctx, "Create and open inventory")) {
      if (nk_strlen(name_buffer) != 0) {
        inv_create_inventory(ctx, &name_buffer, tags_buffer, tag_count);
        ...
        int i;
        for (i = 0; i < tag_count; i++) {
          // printf("tags[%d]: %s", i, tags_buffer[i]);
          // memset(tags_buffer[i], ' ', sizeof(tags_buffer[i]));
          tags_len = 0;
        }
...
init_new_product_window(struct nk_context *ctx)
{
  ...
  if (nk_begin(ctx, "New Product", nk_rect(25, 25, 270, 550), new_pdt_flags))
  {
    static const float ratio[] = {80, 150};
    static char title_buffer[25];
    static char price_buffer[6];
    static char quant_buffer[8];
    ...
    nk_edit_string(ctx, NK_EDIT_FIELD, title_buffer, &text_len[0], 42, nk_filter_default);
    nk_label(ctx, "Price:", NK_TEXT_LEFT);
    nk_edit_string(ctx, NK_EDIT_FIELD, price_buffer, &text_len[1], 42, nk_filter_decimal);
    nk_label(ctx, "Quantity:", NK_TEXT_LEFT);
    nk_edit_string(ctx, NK_EDIT_FIELD, quant_buffer, &text_len[2], 42, nk_filter_float);
    nk_label(ctx, "Description:", NK_TEXT_LEFT);
    nk_layout_row_static(ctx, 120, 233, 1);
    nk_edit_string(ctx, NK_EDIT_BOX, desc_buffer, &desc_len, DESC_MAX, nk_filter_default);
    // TODO - add tag selection

    nk_layout_row_static(ctx, 25, 233, 1);
    if (nk_button_label(ctx, "Create Product")) {
      float price = atof(price_buffer);
      int quantity = (int)atof(quant_buffer);
      inv_create_product(&title_buffer, &desc_buffer, NULL, price, quantity);
    }
  }
  nk_end(ctx);
